...
trachoma_Geoconnect_to_IUID = read.csv("trachoma_IU_Match_PB.csv") # PB's lookup table fo mapping Geoconnect_ID to IU_ID
trachoma_data_raw = read.csv("trachomaComb_IU.csv") %>%
...
save(trachoma_espen_data,file="trachoma_espen_data.Rdata")
...
save(trachoma_data,file="trachoma_data.Rdata")
...
save(iu_task_lookup, file="iu_task_lookup.Rdata")
...
trachoma_all = data.frame(IU_ID = rep(ius_vector,each=length(years_vector)),
...
save(years_vector,file="trachoma_map_years.rds")
save(trachoma_maps,file="trachoma_maps.rds")
...
for (id in 1:max(iu_task_lookup$TaskID)){
  ...
  for (iu in ius[1]) {
    ...
    mda_path<-file.path(inputs_path,paste0("InputMDA_MTP_",id,".csv"))
