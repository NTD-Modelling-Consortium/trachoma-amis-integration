...
prevalence_map <- lapply(1:length(prevalence_map), function(t) {
  output <- list(
    data = as.matrix(prevalence_map[[t]]$data %>%
      filter(TaskID == id) %>%
...
save(infections, file = file.path(kPathToInfections, paste0("infections", id, ".Rdata")))
...
transmission_model <- function(seeds, params, n_tims) {
  ...
  load(file.path(kPathToTrajectories, paste0("trajectories_", id, ".Rdata")))
  ...
  load(file.path(kPathToInfections, paste0("infections", id, ".Rdata")))
...
if (!dir.exists(kPathToAmisOutput)) {
  dir.create(kPathToAmisOutput)
}
save(amis_output, file = file.path(kPathToAmisOutput, paste0("amis_output", id, ".Rdata")))
...
if (n_failure > 0) {
  cat(paste(failures, id, ess[failures]), file = paste0("ESS_NOT_REACHED.txt"), sep = "\n", append = TRUE)
}
if (!file.exists(paste0("summary.csv"))) {
  cat("ID,n_failure,n_success,n_sim,min_ess,duration_amis,durarion_subsampling\n", file = paste0("summary.csv"))
}
